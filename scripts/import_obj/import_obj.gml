/// @function import_obj
/// @param obj_filename		- (required) OBJ file to read
/// @param mtl_filename		- (optional) MTL file to read
/// @param format			- (required) Vertex Format to submit vertices to
/// @desc - Import vertex information from .obj Filetype
///		Takes in a filepath to the obj file,an optional path toan MTL file (use -1 if no MTL file is provided
///		Generates a vertex buffer, populates with vertex Position, UV, and Normal data, and returns the buffer
function import_obj(obj_name, mtl_filename, format){
	...
	//buffer_save(buffer_convert, "%USERFOLDER%\\Documents\\GameMakerStudio2\\Framework3d\\datafiles\\objbins\\" + string(obj_name) + ".vbuff");
